import { EActionConditionOperator, EActionStatus, EErrorOptions, IAction, IActionCondition, IActionStatement, IUserScript, TActionResult, TGoto } from '@dhruv-techapps/acf-common';
...
const Statement = (() => {
  ...
  const checkThen = (condition: boolean, option: EErrorOptions, goto?: TGoto): TActionResult | null => {
    ...
    if (!condition) {
      if (option === EErrorOptions.GOTO && goto) {
        return { status: EErrorOptions.GOTO, goto };
