...
<Callout type="info">
  Bulk Export vs Export Configuration: - Bulk Export creates one JSON file containing all configurations (array format).
  - Export Configuration saves only the currently selected configuration (single-object JSON). - Imports auto-detect the
  file type: arrays trigger bulk import; objects trigger single import. - Imports merge into existing configurations;
  nothing is fully replaced, and duplicates may occur.
</Callout>
