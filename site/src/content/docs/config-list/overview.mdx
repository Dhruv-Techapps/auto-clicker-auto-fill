...
<BsTable>
  | Feature | Description |
  |-------------------------|-----------------------------------------------------------------------------| | New Config
  | Create a new configuration from the top-left action. | | View by Name/URL | Toggle the list to show Name, URL, or
  both. | | Search | Filter configurations by Name or URL for quick access. | | View Details | Select a configuration to
  see details in the right-hand panel. | | Delete | Hover to reveal the delete icon (leftmost) and click to remove an
  item. | | Toggle View | Switch display format (Name, URL, or both) to reduce clutter. |
</BsTable>
