...
<BsTable>
  ...
  |----------------|---------------|-----------------------------------------------------| | `Stop` | selected | Stop
  the workflow; no further actions run. | | `Skip Action` | not selected | Skip this action and continue with the next
  one. | | `Refresh` | not selected | Reload the current page before continuing. | | `Goto Action` | not selected | Jump
  to a specific action in the list. |
</BsTable>
