...
<Callout type="info">
  ### Important Notes: - Execution waits until the specified time of day; it does not run immediately after a page
  refresh. - Supported in Auto mode only. In Manual mode, the task is triggered by a user-defined hotkey.
</Callout>
