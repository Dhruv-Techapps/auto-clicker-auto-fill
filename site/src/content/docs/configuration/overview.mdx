...
toc: true
tags: [configuration, overview, name, init-wait, enable, export, import, duplicate]
aliases:
  - '/docs/3.1/configuration/export-import/'
sections:
  - title: Batch
    description: Manage multiple configurations at once for streamlined operations.
  - title: Watch
  ...
  - title: Config Settings
    description: Adjust configuration-level settings to control behavior and preferences.
  - title: URL
    description: Specify the target URL or pattern for the configuration to match.
---
...
- **Details**:
  ...
  - Detects browser back/forward navigation (`popstate` events).
  - Detects hash-based navigation (`hashchange` events).
  - Detects programmatic navigation using History API (`pushState`, `replaceState`).
