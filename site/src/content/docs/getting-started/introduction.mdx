...
3. Right‑click any element (`button`, `input`, `textarea`, etc.) and choose <kbd>[[config:title]]</kbd>.
...
- Use right‑click → <kbd>[[config:title]]</kbd> → `★ Configure for this Field` to auto‑capture the [Element Finder]([[docsref:/action/element-finder]]).
