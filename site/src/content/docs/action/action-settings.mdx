...
<Callout type="info">
  Sequential restart modes are essential for complex workflows like custom dropdowns where Action 1 opens the dropdown, Action 2 filters options, and Action 3 selects the result. When new options appear, "From This Action" mode ensures the complete workflow runs correctly.
</Callout>
...
<Callout type="warning">
  Sequential restart modes will re-execute multiple actions, which may impact performance on pages with frequent DOM changes. Use appropriate debounce timing to limit restart frequency.
</Callout>
