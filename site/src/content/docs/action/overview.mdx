...
<Callout type="info">
  **New: DOM Watcher** - Enable automatic re-execution of actions when new matching elements are added to the page.
  Perfect for infinite scrolling feeds, live comments, and dynamic content. Configure via [Action
  Settings]([[docsref:/action/action-settings#dom-watcher-settings]]) → DOM Watcher Settings.
</Callout>
